...
# Import coordinates
df = pd.read_csv("coords_and_velBack_" + str(no) + ".csv")
x = np.array(df["x"])
y = np.array(df["y"])
z = np.array(df["z"])
t = np.array(df["t"])
...
for time in escTimes:
    if (counter < 7):
        print("Escape time for %s = %0.2f million years ago"%(objects[counter], -escTimes[counter]/1e6))
    else:
        print("Escape time for asteroid %d clone %d = %0.2f million years ago"%(no, counter - 7, -escTimes[counter]/1e6))
    counter += 1